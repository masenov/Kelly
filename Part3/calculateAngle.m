function [x,y,phi,current_time] = calculateAngle(s,goal_x,goal_y,x,y,phi,current_time,av_sensors,high_threshold)
    if (av_sensors(4) > high_threshold)
        angle = mod(phi-90,360);
    else
        angle = phi;
    end
    difference = directionDifference(goal_x,goal_y,x,y,angle)/53;
